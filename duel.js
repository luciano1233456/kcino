// ========== SISTEMA 1VS1 - TIRO AL BLANCO (VERSIÓN CORREGIDA) ==========
(function() {
    ...
    // ALMACENAR PUNTAJE DEL JUGADOR PARA COMPARAR
    let myScore = 0;
    ...
    function showAimDuel() {
        ...
        modal.innerHTML = `
            <div style="background: #1a1a1a; border-radius: 30px; padding: 20px; max-width: 900px; width: 95%; height: 80vh; text-align: center; border: 3px solid #ff44ff; display: flex; flex-direction: column;">
                <h2 style="color: #ff44ff; margin-bottom: 10px;">🎯 TIRO AL BLANCO VS ${opponentName} 🎯</h2>
                <p style="color: #ffd700; margin-bottom: 10px;">💰 Apuesta: $${duelStake.toLocaleString()}</p>
                <div style="display: flex; justify-content: space-between; margin-bottom: 15px; padding: 0 20px;">
                    <div style="background: #0a0a0a; padding: 10px 20px; border-radius: 15px;">
                        <span style="color: #aaa;">🎯 TUS ACIERTOS</span>
                    ...
                    <div style="background: #0a0a0a; padding: 10px 20px; border-radius: 15px;">
                        ...
                        <span id="timerCount" style="color: #ffd700; font-size: 32px; font-weight: bold; margin-left: 10px;">10</span>
                    </div>
                </div>
                ...
                <p style="color: #aaa; font-size: 12px; margin-top: 10px;">🎯 Haz clic en los objetivos rojos para sumar puntos. ¡Tienes 10 segundos!</p>
        ...
        gameTimer = setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                // ENVIAR PUNTAJE AL SERVIDOR Y ESPERAR RESULTADO DEL RIVAL
                endDuelGame();
            }
        }, 1000);
    }
    
    // ========== NUEVA FUNCIÓN: COMPARAR CON EL RIVAL ==========
    async function endDuelGame() {
        ...
        // Mostrar pantalla de carga mientras se compara
        modal.innerHTML = `
            <div style="background: #1a1a1a; border-radius: 30px; padding: 40px; max-width: 450px; width: 90%; text-align: center; border: 3px solid #ffd700;">
                <div style="font-size: 48px; animation: pulse 1s infinite;">⚔️</div>
                <h2 style="color: #ffd700; margin: 20px;">¡Comparando resultados!</h2>
                <p style="color: #aaa;">Esperando resultado de ${opponentName}...</p>
                <div style="margin-top: 20px;">
                    <div style="width: 100%; height: 4px; background: #333; border-radius: 2px; overflow: hidden;">
                        <div style="width: 0%; height: 100%; background: #ff44ff; animation: loading 2s infinite;"></div>
                    </div>
                </div>
                <p style="color: #ff44ff; margin-top: 20px;">Tu puntaje: ${myFinalScore} 🎯</p>
            </div>
            <style>
        ...
        // GUARDAR PUNTAJE EN LA SALA PARA QUE EL RIVAL LO VEA
        await window.saveDuelScore(user, opponentName, myFinalScore, duelStake);
        
        // ESPERAR A QUE EL RIVAL TERMINE (máximo 15 segundos)
        let attempts = 0;
        const maxAttempts = 30;
        
        const checkInterval = setInterval(async () => {
            attempts++;
            const result = await window.getDuelResult(user, opponentName);
            
            if (result && result.opponentScore !== undefined) {
                clearInterval(checkInterval);
                
                const opponentFinalScore = result.opponentScore;
                const won = myFinalScore > opponentFinalScore;
                const tie = myFinalScore === opponentFinalScore;
                ...
                if (tie) {
                    ...
                    modal.innerHTML = `
                        <div style="background: #1a1a1a; border-radius: 30px; padding: 30px; max-width: 500px; width: 90%; text-align: center; border: 3px solid #ffd700;">
                            <h2 style="color: #ffd700;">🤝 ¡EMPATE! 🤝</h2>
                            <div style="font-size: 48px; margin: 20px;">😐</div>
                            <p style="color: #ffd700; font-size: 18px;">Tu puntaje: ${myFinalScore} 🎯</p>
                            <p style="color: #aaa;">Puntaje de ${opponentName}: ${opponentFinalScore} 🎯</p>
                            <p style="color: #ffd700; margin: 15px;">Recuperas tu apuesta de $${duelStake.toLocaleString()}</p>
                    ...
                    `;
                    showNotification(`🤝 Empate con ${opponentName}! Recuperas $${duelStake.toLocaleString()}`, true);
                    addHistoryLog(`🎯 DUELO vs ${opponentName}: EMPATE (${myFinalScore} vs ${opponentFinalScore})`, 'event');
                    
                } else if (won) {
                    const winAmount = duelStake * 2;
                    ...
                    addHistoryLog(`🎯 DUELO vs ${opponentName}: GANASTE $${winAmount.toLocaleString()} (${myFinalScore} - ${opponentFinalScore})`, 'win');
                    
                    // Transferir dinero del perdedor
                    await window.transferMoney(opponentName, user, duelStake);
                    ...
                    modal.innerHTML = `
                        <div style="background: #1a1a1a; border-radius: 30px; padding: 30px; max-width: 500px; width: 90%; text-align: center; border: 3px solid #ff4444;">
                            ...
                            <p style="color: #ffd700; font-size: 18px;">Tu puntaje: ${myFinalScore} 🎯</p>
                            <p style="color: #aaa;">Puntaje de ${opponentName}: ${opponentFinalScore} 🎯</p>
                            <p style="color: #ff4444; font-size: 28px; margin: 15px;">-$${duelStake.toLocaleString()}</p>
                            <button onclick="cerrarDueloYActualizar()" style="margin-top: 20px; background: #ffd700; padding: 10px 30px; border-radius: 10px; cursor: pointer;">Cerrar</button>
                        </div>
                    `;
                    showNotification(`💀 Perdiste el duelo contra ${opponentName}`, false);
                    addHistoryLog(`🎯 DUELO vs ${opponentName}: PERDISTE $${duelStake.toLocaleString()} (${myFinalScore} - ${opponentFinalScore})`, 'loss');
                    updateUI();
                    await saveGame();
                }
                
                // Limpiar el resultado de la sala
                await window.clearDuelResult(user, opponentName);
                ...
                clearInterval(checkInterval);
                // Timeout - el rival no terminó, se considera victoria automática
                const winAmount = duelStake * 2;
                window.money += winAmount;
                updateUI();
                await saveGame();
                ...
                modal.innerHTML = `
                    <div style="background: #1a1a1a; border-radius: 30px; padding: 30px; max-width: 500px; width: 90%; text-align: center; border: 3px solid #44ff44;">
                        <h2 style="color: #44ff44;">🎉 ¡VICTORIA POR INACTIVIDAD! 🎉</h2>
                        <p style="color: #ffd700; font-size: 18px;">Tu puntaje: ${myFinalScore} 🎯</p>
                        <p style="color: #44ff44; font-size: 28px;">+$${winAmount.toLocaleString()}</p>
                        <button onclick="cerrarDueloYActualizar()" style="margin-top: 20px; background: #ffd700; padding: 10px 30px; border-radius: 10px;">Cerrar</button>
                    </div>
                `;
                await window.clearDuelResult(user, opponentName);
                inDuel = false;
